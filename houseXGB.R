...
#Identifying Noise Columns from training set using Boruta algorithm
# b_out = Boruta(formula = SalePrice~.,
#                data = training_dataset,
#                doTrace = 1)
# b_sig = getSelectedAttributes(b_out, withTentative = FALSE)
# plot(b_out, cex.axis=.7, las=2, xlab="", main="Variable Importance")

#Removing Noise columns from training and test sets
sale_price = training_dataset$SalePrice
training_dataset = subset(training_dataset, select=c( "MSSubClass", "MSZoning", "LotFrontage", "LotArea", "LotShape",
                                                      "Neighborhood", "BldgType", "HouseStyle", "OverallQual", "OverallCond", "YearBuilt", "YearRemodAdd",
                                                      "Exterior1st", "Exterior2nd", "MasVnrType", "MasVnrArea", "ExterQual", "Foundation", "BsmtQual", "BsmtCond",
                                                      "BsmtFinType1", "BsmtFinSF1", "BsmtFinType2", "BsmtUnfSF", "TotalBsmtSF", "HeatingQC", "CentralAir",
                                                      "X1stFlrSF","X2ndFlrSF", "GrLivArea", "BsmtFullBath", "FullBath", "HalfBath", "BedroomAbvGr",
...
test_dataset = subset(test_dataset, select=c( "MSSubClass", "MSZoning", "LotFrontage", "LotArea", "LotShape",
                                              "Neighborhood", "BldgType", "HouseStyle", "OverallQual", "OverallCond", "YearBuilt", "YearRemodAdd",
                                              "Exterior1st", "Exterior2nd", "MasVnrType", "MasVnrArea", "ExterQual", "Foundation", "BsmtQual", "BsmtCond",
                                              "BsmtFinType1", "BsmtFinSF1", "BsmtFinType2", "BsmtUnfSF", "TotalBsmtSF", "HeatingQC", "CentralAir", 
                                              "X1stFlrSF","X2ndFlrSF", "GrLivArea", "BsmtFullBath", "FullBath", "HalfBath", "BedroomAbvGr",
                                              "KitchenAbvGr", "KitchenQual", "TotRmsAbvGrd", "Functional", "Fireplaces", "FireplaceQu", "GarageType", "GarageYrBlt",
                                              "GarageFinish", "GarageCars", "GarageArea", "GaarageQual", "GarageCond", "PavedDrive", "WoodDeckSF", "OpenPorchSF"))

#Building Extrememe Gradient Boosting Model
xgb = xgboost(data = data.matrix(training_dataset[, -1]),
              n=250,
              label = sale_price)

#Predicting Prices of houses
y_pred = data.frame(predict(xgb, data.matrix(test_dataset[, -1])))

#Making Graphs of the data
x_grid = seq(min(training_dataset$LotArea), max(training_dataset$LotArea), 1)
ggplot() + 
  geom_point(aes(x = training_dataset$LotArea, y = sale_price), colour = 'red') +
  geom_line(aes(x = x_grid, y = predict(xgb, newdata = xgb.DMatrix(LotArea = x_grid))), colour = 'blue') + 
